...
<form id="formAdd" action="index.php?action=add" method="post">
    <div class="input">
...
if (count($res) != 0){
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        if (!TaskHandler::compareDates(date("d/m/Y"), $deadline))
            $m++;
    }
    ...
    for ($i = 0; $i < count($res); $i++){
        $deadline = date("d/m/Y", strtotime($res[$i]["deadLine"]));
        $now = date("d/m/Y");
        if (!TaskHandler::compareDates($now, $deadline))
        ...
        <div class="task {$class}">
            ...
            <form method="post" action="index.php?action=del">
